# Code adapted from https://github.com/bulletphysics/bullet3
...
class SwolKatEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self._is_render:
            ...
            targetPos = [
                ...
                0.95 * curTargetPos[1] + 0.05 * base_pos[1],
                curTargetPos[2],
            ]
    ...
    def is_fallen(self):
        ...
        return (
            np.dot(np.asarray([0, 0, 1]), np.asarray(local_up)) < 0.85 or pos[2] < 0.13
        )
    ...
    def _reward(self):
        ...
        energy_reward = (
            np.abs(
                np.dot(
                    self.minitaur.GetMotorTorques(), self.minitaur.GetMotorVelocities()
                )
            )
            * self._time_step
        )
